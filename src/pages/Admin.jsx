...
const MRR_TREND = [
  { month:'Jan', mrr:498 }, { month:'Feb', mrr:577 }, { month:'Mar', mrr:656 },
  { month:'Apr', mrr:735 }, { month:'May', mrr:756 }, { month:'Jun', mrr:956 },
]
...
export default function Admin() {
  ...
  const [flags,        setFlags]        = useState(() => storage.load('admin_flags', FEATURE_FLAGS_DEFAULT))
  const [toast,        setToast]        = useState(null)
  const [emailSection, setEmailSection] = useState(false)

  // Email settings (persisted)
  const [supportEmail,  setSupportEmail]  = useState(() => storage.load('settings', {})?.supportEmail  || '')
  const [contactEmail,  setContactEmail]  = useState(() => storage.load('settings', {})?.contactEmail  || '')

  useEffect(() => { storage.save('admin_flags', flags) }, [flags])
  ...
  const saveEmailSettings = () => {
    const current = storage.load('settings', {})
    storage.save('settings', { ...current, supportEmail, contactEmail })
    ...
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:"'Inter',sans-serif", color:C.white }}>
      ...
      <div style={{ padding:28, maxWidth:1300, margin:'0 auto' }}>
        <div style={{ marginBottom:28 }}>
          ...
          <div style={{ fontSize:13, color:C.muted, marginTop:4 }}>All tenants, billing, and platform settings.</div>
        </div>
          ...
          {[
            ...
            <div key={i} style={{ background:C.card, borderRadius:14, padding:18, border:`1px solid ${C.border}` }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10 }}>
                <span style={{ fontSize:11, color:C.muted }}>{s.l}</span>{s.i}
              </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:20, marginBottom:24 }}>
          ...
          <div style={{ background:C.card, borderRadius:16, padding:20, border:`1px solid ${C.border}` }}>
            ...
            {[['Enterprise','enterprise',C.purple],['Pro','pro',C.blue],['Starter','starter',C.muted]].map(([label,key,color]) => {
              const count  = tenants.filter(t => t.plan === key).length
              const rowMrr = tenants.filter(t => t.plan === key).reduce((s,t) => s+t.mrr, 0)
              return (
                <div key={key} style={{ marginBottom:16 }}>
                  ...
                  <div style={{ height:6, background:C.card2, borderRadius:3, overflow:'hidden' }}>
                    <div style={{ height:'100%', width:`${(count/tenants.length)*100}%`, background:color, borderRadius:3, transition:'width .5s' }}/>
                  </div>
                </div>
              )
            })}
        ...
        <div style={{ background:C.card, borderRadius:16, padding:24, border:`1px solid ${C.border}`, marginBottom:24 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
            ...
            <button onClick={() => notify('Invite flow — connect to your auth provider')} style={{ padding:'8px 16px', borderRadius:8, border:'none', background:C.purple, color:C.white, cursor:'pointer', fontWeight:600, fontSize:13, fontFamily:'inherit' }}>+ Invite Tenant</button>
          </div>
          ...
          {tenants.map(t => (
            <div key={t.id} style={{ display:'grid', gridTemplateColumns:'2fr 1fr 0.7fr 0.7fr 0.8fr 0.8fr', gap:0, padding:'13px 0', borderBottom:`1px solid ${C.border}`, alignItems:'center' }}>
              ...
              <select value={t.plan} onChange={e => setPlan(t.id, e.target.value)} style={{ fontSize:10, fontWeight:700, padding:'4px 8px', borderRadius:7, border:'none', cursor:'pointer', background:`${PLAN_C[t.plan]}28`, color:PLAN_C[t.plan], outline:'none', fontFamily:'inherit' }}>
              ...
              <div style={{ fontSize:13, fontWeight:700, color:C.green }}>{t.mrr > 0 ? `€${t.mrr}` : <span style={{ color:C.dim }}>—</span>}</div>
        ...
        {/* Email Settings — KEY NEW SECTION */}
        <div style={{ background:C.card, borderRadius:16, padding:24, border:`1px solid ${C.border}`, marginBottom:24 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, cursor:'pointer' }} onClick={() => setEmailSection(e => !e)}>
            ...
            <div style={{ fontSize:20, color:C.muted, transform:emailSection?'rotate(180deg)':'none', transition:'transform .2s' }}>▾</div>
          </div>

          {emailSection && (
            <div style={{ animation:'fadeUp .2s ease' }}>
              <div style={{ background:`${C.purple}0A`, border:`1px solid ${C.purple}33`, borderRadius:12, padding:16, marginBottom:18, fontSize:13, color:C.muted }}>
                💡 Diese E-Mail-Adressen empfangen die Formular-Einsendungen von der Webseite. Füge außerdem <code style={{ background:C.card2, padding:'2px 6px', borderRadius:4, color:C.blue }}>RESEND_API_KEY</code> + <code style={{ background:C.card2, padding:'2px 6px', borderRadius:4, color:C.blue }}>CONTACT_EMAIL</code> in den Vercel Environment Variables hinzu.
              </div>

              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
                <div>
                  <label style={{ fontSize:11, color:C.muted, display:'block', marginBottom:8, fontWeight:600, letterSpacing:1 }}>
                    📬 KONTAKT / ENTERPRISE-ANFRAGEN
                  </label>
                ...
                <div>
                  ...
                  <div style={{ position:'relative' }}>
                    ...
                    <input
                      ...
                      placeholder="support@scenvy.de"
                      type="email"
                      style={{ width:'100%', padding:'11px 14px 11px 34px', borderRadius:9, border:`1px solid ${C.border}`, background:C.bg, color:C.white, fontSize:13, outline:'none', fontFamily:'inherit' }}
                    />
                  </div>
                  <div style={{ fontSize:11, color:C.dim, marginTop:5 }}>Empfängt Support-Anfragen aus dem Help-Center</div>
                </div>
              </div>

              <button onClick={saveEmailSettings} style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 22px', borderRadius:10, border:'none', background:grad(C.purple,C.pink), color:C.white, cursor:'pointer', fontWeight:700, fontSize:14, fontFamily:'inherit' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:20 }}>
          <div style={{ background:C.card, borderRadius:16, padding:24, border:`1px solid ${C.border}` }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12 }}>
              {flags.map((f, i) => (
                <div key={i} onClick={() => toggleFlag(i)} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 14px', background:C.card2, borderRadius:10, cursor:'pointer', border:`1px solid ${f.on ? f.c+'44' : C.border}`, transition:'border .2s' }}>
          ...
          <div style={{ background:C.card, borderRadius:16, padding:24, border:`1px solid ${C.border}` }}>
            ...
            {[
              { label:'API Response',  value:'48ms',   ok:true  },
              { label:'CDN (Videos)',  value:'99.9%',  ok:true  },
              { label:'AI Generator', value:'Online', ok:true  },
              { label:'Auth Service', value:'Online', ok:true  },
              { label:'DB (Postgres)',value:'12ms',   ok:true  },
              { label:'Email Service',value: contactEmail ? 'Configured ✓' : 'Not configured', ok:!!contactEmail },
            ].map((s, i) => (
              <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderBottom:i < 5 ? `1px solid ${C.border}` : 'none' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  <div style={{ width:6, height:6, borderRadius:'50%', background:s.ok ? C.green : C.orange }}/>
                  <span style={{ fontSize:13, fontWeight:600, color:s.ok ? C.green : C.orange }}>{s.value}</span>
                </div>
              </div>
            ))}
        ...
        <div style={{ marginTop:24, background:C.card, borderRadius:16, padding:24, border:`1px solid ${C.border}` }}>
          ...
          <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
            {[
              ['📧 Send Announcement',  () => notify('Email modal — connect SendGrid or Resend')],
              ['💾 Export All Data',     () => notify('CSV export — connect to your DB')          ],
              ['🔑 Reset API Keys',      () => notify('API key rotation — set in backend')         ],
              ['🌍 CDN Purge',           () => notify('CDN cache purged')                          ],
              ['⚙️ Maintenance Mode',   () => notify('Maintenance mode toggled')                  ],
            ].map(([label, action], i) => (
              <button key={i} onClick={action} style={{ padding:'10px 18px', borderRadius:9, border:`1px solid ${C.border}`, background:C.card2, color:C.muted, cursor:'pointer', fontSize:13, fontWeight:500, fontFamily:'inherit', transition:'color .15s, border-color .15s' }}
